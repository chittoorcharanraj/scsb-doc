...
public class MatchingAlgorithmControllerUT extends BaseTestCaseUT4 {
    ...
    public void matchingAlgorithmFindMatchingAndReportsTest() throws Exception {
        ...
        Mockito.when(matchingAlgorithmHelperService.populateReportsForMatchPoints(batchSize, ScsbCommonConstants.MATCH_POINT_FIELD_OCLC, ScsbCommonConstants.MATCH_POINT_FIELD_ISBN, getStringIntegerMap(), MatchScoreUtil.OCLC_ISBN_SCORE)).thenReturn(getStringIntegerMap());
        Mockito.when(matchingAlgorithmHelperService.populateReportsForMatchPoints(batchSize, ScsbCommonConstants.MATCH_POINT_FIELD_OCLC, ScsbCommonConstants.MATCH_POINT_FIELD_ISSN, getStringIntegerMap(), MatchScoreUtil.OCLC_ISBN_SCORE)).thenReturn(getStringIntegerMap());
        Mockito.when(matchingAlgorithmHelperService.populateReportsForMatchPoints(batchSize, ScsbCommonConstants.MATCH_POINT_FIELD_OCLC, ScsbCommonConstants.MATCH_POINT_FIELD_LCCN, getStringIntegerMap(), MatchScoreUtil.OCLC_ISBN_SCORE)).thenReturn(getStringIntegerMap());
        Mockito.when(matchingAlgorithmHelperService.populateReportsForMatchPoints(batchSize, ScsbCommonConstants.MATCH_POINT_FIELD_ISBN, ScsbCommonConstants.MATCH_POINT_FIELD_ISSN, getStringIntegerMap(), MatchScoreUtil.OCLC_ISBN_SCORE)).thenReturn(getStringIntegerMap());
        Mockito.when(matchingAlgorithmHelperService.populateReportsForMatchPoints(batchSize, ScsbCommonConstants.MATCH_POINT_FIELD_ISBN, ScsbCommonConstants.MATCH_POINT_FIELD_LCCN, getStringIntegerMap(), MatchScoreUtil.OCLC_ISBN_SCORE)).thenReturn(getStringIntegerMap());
        Mockito.when(matchingAlgorithmHelperService.populateReportsForMatchPoints(batchSize, ScsbCommonConstants.MATCH_POINT_FIELD_ISSN, ScsbCommonConstants.MATCH_POINT_FIELD_LCCN, getStringIntegerMap(), MatchScoreUtil.OCLC_ISBN_SCORE)).thenReturn(getStringIntegerMap());
    ...
    public void testItemCountForSerials() throws Exception {
        ...
        Mockito.when(matchingAlgorithmUpdateCGDService.getMatchingAlgorithmReportDataDetailsRepository()).thenReturn(reportDataDetailsRepository);
        ...
        Mockito.when(matchingAlgorithmUpdateCGDService.getMatchingAlgorithmReportDataDetailsRepository().getCountOfRecordNumForMatchingSerials(ScsbCommonConstants.BIB_ID)).thenReturn(Long.valueOf(10000));
        for(int pageNum = 0; pageNum < totalPagesCount + 1; pageNum++) {
            long from = pageNum * Long.valueOf(batchSize);
            Mockito.when(matchingAlgorithmUpdateCGDService.getMatchingAlgorithmReportDataDetailsRepository().getReportDataEntityForMatchingSerials(ScsbCommonConstants.BIB_ID, from, batchSize)).thenReturn(Arrays.asList(reportDataEntity));
        }
        Mockito.when(matchingAlgoController.itemCountForSerials()).thenCallRealMethod();
        String response = matchingAlgoController.itemCountForSerials();
        assertTrue(response.contains("Items Count"));
