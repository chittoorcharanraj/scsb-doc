...
public class S3SubmitCollectionSuccessReportRouteBuilder {
     ...
     */
    @Autowired
    public S3SubmitCollectionSuccessReportRouteBuilder(CamelContext context, @Value("${" + PropertyKeyConstants.S3_ADD_S3_ROUTES_ON_STARTUP + "}") boolean addS3RoutesOnStartup, @Value("${" + PropertyKeyConstants.S3_SUBMIT_COLLECTION_SUPPORT_TEAM_REPORT_DIR + "}") String submitCollectionS3ReportPath) {
        try {
            if (addS3RoutesOnStartup) {
                context.addRoutes(new RouteBuilder() {
                    @Override
                    public void configure() throws Exception {
                        from(ScsbConstants.FTP_SUBMIT_COLLECTION_SUCCESS_REPORT_Q)
                                .routeId(ScsbConstants.FTP_SUBMIT_COLLECTION_SUCCESS_REPORT_ID)
