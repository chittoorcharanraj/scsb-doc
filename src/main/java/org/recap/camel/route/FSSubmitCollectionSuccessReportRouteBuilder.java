...
public class FSSubmitCollectionSuccessReportRouteBuilder {
     ...
     */
    @Autowired
    public FSSubmitCollectionSuccessReportRouteBuilder(CamelContext context, @Value("${" + PropertyKeyConstants.SUBMIT_COLLECTION_REPORT_DIRECTORY + "}") String reportsDirectory) {
        try {
            context.addRoutes(new RouteBuilder() {
                @Override
                public void configure() throws Exception {
                    from(ScsbConstants.FS_SUBMIT_COLLECTION_SUCCESS_REPORT_Q)
                            .routeId(ScsbConstants.FS_SUBMIT_COLLECTION_SUCCESS_REPORT_ID)
