...
public interface MatchingAlgorithmReportDataDetailsRepository extends BaseRepository<MatchingAlgorithmReportDataEntity> {
    ...
    @Query(value = "select * from ma_report_data_t\n" +
            ...
            "(select record_num from ma_report_t where date(CREATED_DATE)=?1 and type like %?2%) and HEADER_VALUE like %?3%) and HEADER_NAME='BibId' and HEADER_VALUE like %?4% ;", nativeQuery = true)
