...
public interface BibliographicDetailsRepository extends BaseRepository<BibliographicEntity> {
    ...
    @Modifying
    @Query(value = "UPDATE `BIBLIOGRAPHIC_T` AS a " +
            "inner JOIN (select distinct(b1.MATCHING_IDENTITY) from bibliographic_t b1, bibliographic_t b2 " +
            "where b1.MATCH_SCORE <> b2.MATCH_SCORE and b1.MATCHING_IDENTITY = b2.MATCHING_IDENTITY and b1.BIBLIOGRAPHIC_ID in (:bibliographicIds) " +
            "group by b1.MATCHING_IDENTITY ) AS b ON a.MATCHING_IDENTITY = b.MATCHING_IDENTITY", nativeQuery = true)
    @Transactional
